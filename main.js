...
// 数値を通貨書式「#,###,###」に変換するフィルター
// フィルターは、そのフィルターを利用するコンポーネントよりも先に定義する。
  ...
  computed: {
    ...
    filteredList: function() {
      ...
      for (var i=0; i<this.products.length; i++) {
        ...
        // 条件を満たしている商品だけpushして新しい配列を積み込んでいく
        if (isShow) {
      ...
      // ループが終われば、絞り込み後の商品リストをreturnで返す
      return newList;
    }
  }
});

// 絞り込み機能を実装するには、ユーザーがチェックした状態を更新したタイミングなのでv-onディレクティブを使ってchangeイベントをハンドリングする
// データの変更を監視してくれるのでwatchを使う。もしくはcomputed(算出プロパティ)を使用して何らかの加工を行った結果を返すプロパティを使うのもOK
